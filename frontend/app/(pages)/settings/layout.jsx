...
const NavButton = ({ iconClass, href = "", content = "" }) => {
  ...
  return (
    <Link
      className={clsx(
        "flex h-full items-center text-center py-2 px-4 rounded-lg dark:hover:bg-gray-500 dark:bg-black hover:bg-gray-400 bg-gray-200 transition-colors w-full",
        {
          "!bg-gray-500 text-white hover:text-black dark:hover:text-white":
            pathname === href,
        }
      )}
      href={href}
...
const Title = ({ content = "" }) => {
  return <p className="my-3 text-gray-500">{content}</p>;
};
