...
const VideoCallPage = () => {
  ...
  const [gradient, setGradient] = useState(
    "radial-gradient(circle, #4B5EAA80, #1A1A3D80)"
  );
  ...
  useEffect(() => {
    if (stream && myScreen.current && !myScreen.current.srcObject) {
      ...
      myScreen.current
        .play()
        .catch((err) => console.error("MyScreen play error:", err));
    }
    if (
      callAccepted &&
      receiverScreen.current &&
      receiverScreen.current.srcObject
    ) {
      console.log("Receiver screen should be playing");
      receiverScreen.current
        .play()
        .catch((err) => console.error("Receiver play error:", err));
    }
      ...
      <div className="w-full h-full flex justify-center items-center absolute top-0 left-0">
        {callAccepted && !callEnded ? (
          ...
          <div className="text-center bg-gray-800 p-6 rounded-lg shadow-lg">
            <h1 className="text-2xl font-bold">{nameReceiver}Calling you...</h1>
          </div>
        ) : callEnded ? (
